...
#include "ros_controllers/tracking_pacejka_mpcConfig.h"
...
class DynamicPacejkaTrackingMPCConfigServer
{
protected:
  std::shared_ptr<crs_controls::pacejka_tracking_mpc::PacejkaTrackingMpcController> controller_;
  dynamic_reconfigure::Server<tracking_pacejka_mpcConfig> server;
  dynamic_reconfigure::Server<tracking_pacejka_mpcConfig>::CallbackType f;

public:
  DynamicPacejkaTrackingMPCConfigServer(
      const ros::NodeHandle& nh,
      std::shared_ptr<crs_controls::pacejka_tracking_mpc::PacejkaTrackingMpcController> controller)
    : server(nh), controller_(controller)
  {
    f = boost::bind(&DynamicPacejkaTrackingMPCConfigServer::callback, this, _1, _2);
    server.setCallback(f);
  };
   ...
   * @param level
   */
  void callback(ros_controllers::tracking_pacejka_mpcConfig& config, uint32_t level)
  {
    if (!controller_)
      return;

    crs_controls::pacejka_tracking_mpc::tracking_mpc_pacejka_config pid_cfg = controller_->getConfig();
    pid_cfg.Q1 = config.Q1;
    pid_cfg.Q2 = config.Q2;
    pid_cfg.R1 = config.R1;
    pid_cfg.R2 = config.R2;
